	...
	<body>
		<% try {
			...
			if (amount.isEmpty()) {
				...
				%>
					<a href="AddBid.jsp"><input onclick="this.parentNode.href=AddBid.jsp;" type="button" value="Go Back" name="Fail"/></a>
				<%
			}else{
				//Make INSERT vehicles query
				PreparedStatement statement = con.prepareStatement("INSERT IGNORE INTO bid (aid, username, amount, increment, upperLim) VALUES (?, ?, ?, ?, ?)");
				...
				String winner = userN;
				Double mxBid = curBid;
				PreparedStatement auto = con.prepareStatement("select username, max(amount), upperlim, increment from bid where increment!=0 and aid=? group by username");
				...
				while (autoList.next()){
					...
					if (curUser.equals(winner)==false && lim > curBid+incr ){
						PreparedStatement autoBid = con.prepareStatement("insert into bid (aid, username, amount, increment, upperlim) value (?,?,?,?,?)");
				...
				<body>
					...
					<a href="GetVehicle.jsp"><input onclick="this.parentNode.href=GetVehicle.jsp;" type="button" value="Go Back to listings" name="goback"/></a>
				</body>
				<%
				}
